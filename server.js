  ...
  try {
    ...
    if (data.photos && data.photos.length > 0) {
      for (var i = 0; i < data.photos.length; i++) {
        ...
        var photoBuffer = Buffer.from(base64Data, 'base64');
        var photoPath = 'BSC Inspections/' + folder + '/Photos/' + fileName + '/' + photoName;
        await uploadFile(token, photoPath, photoBuffer, photo.type || 'image/jpeg');
      }
    }
...
app.post('/photos', async (req, res) => {
  try {
    ...
    // List subfolders (one per submission)
    ...
    // Get photos from matching folder(s)
    var allPhotos = [];
    for (var folder_item of folders) {
      const imgListUrl  = 'https://graph.microsoft.com/v1.0/users/' + USER_ID + '/drive/items/' + folder_item.id + '/children?$select=name,id,@microsoft.graph.downloadUrl';
      const imgListResp = await fetch(imgListUrl, { headers: { 'Authorization': 'Bearer ' + token } });
      if (!imgListResp.ok) continue;
      const imgs = (await imgListResp.json()).value || [];
      imgs.forEach(img => {
        allPhotos.push({ name: img.name, url: img['@microsoft.graph.downloadUrl'] });
      });
    }
...
async function appendExcelRow(token, folder, data, fileName) {
  ...
  const values = folder === 'Inward' ? [[
    ...
    // Final measurements — columns M to R
    data.first_bit||'', data.last_bit||'', data.defective||'', data.balance_wt||'',
    data.coil_verified||'', data.blade_clearance||'',
    // Sign off — columns S to U
    data.operator||'', data.inspector||'', data.remarks||'',
    // Quality checklist — columns V to AC
    ...
    // Processed quantity sizes 1-10 — columns AD onwards
