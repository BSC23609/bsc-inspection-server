...
// ── Download PDF endpoint — search by batch, date, or both ───
app.post('/download', async (req, res) => {
  try {
    ...
    const files    = (listJson.value || []).map(f => f.name);

    // Filter files matching batch and/or date
    ...
    if (matches.length === 0) {
      return res.status(404).json({
        status: 'error',
        message: `No report found matching ${batch ? 'batch: ' + batch : ''}${batch && date_str ? ' and ' : ''}${date_str ? 'date: ' + date_str : ''}`
      });
    }
    ...
    const fileUrl  = `https://graph.microsoft.com/v1.0/users/${USER_ID}/drive/root:/${encodeURIComponent(filePath)}:/content`;
